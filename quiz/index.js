...
//https://developer.amazon.com/blogs/alexa/post/9ec7c7d2-a937-4676-b936-48dd2abd0f66/what-s-new-with-request-handling-in-the-alexa-skills-kit-sdk-for-node-js


const Alexa = require('ask-sdk-core'); //helps in creating engaging skills with more focus to features than boiler-plate code

/* INTENT HANDLERS */
const LaunchRequestHandler = {
  canHandle(handlerInput) {   //this handler can handle only for specific request .. the value returned in boolean
    return handlerInput.requestEnvelope.request.type === `LaunchRequest`;
  },
  handle(handlerInput) {   //if boolean is True then do this, pass input and build response accordingly
    return handlerInput.responseBuilder //i think text response
      .speak(welcomeMessage) //voice reponse
      .reprompt(helpMessage) //response if no reply from user
      .getResponse(); //get user reply
  },
};
...
const QuizHandler = {
  canHandle(handlerInput) {
    ...
    console.log("Inside QuizHandler"); //print this to console..
    console.log(JSON.stringify(request)); //converting request into json and op to console
    return request.type === "IntentRequest" &&
           (request.intent.name === "QuizIntent" || request.intent.name === "AMAZON.StartOverIntent");
  },
  handle(handlerInput) {
    console.log("Inside QuizHandler - handle");//attributesManager - An interface handling three level attributes: request, session and persistence.
    const attributes = handlerInput.attributesManager.getSessionAttributes();//Provides session attributes extracted from request envelope. | Type parameters T | Returns T

    const response = handlerInput.responseBuilder;//method returns a ResponseBuilder object. Use the methods on this object to create a response with the text to speak and the card to display.
    ...
    const item = attributes.quizItem;//1
    const property = attributes.quizProperty;//2

    if (supportsDisplay(handlerInput)) {//looks like a ppt presentation..have to first enable display
      const title = `Question #${attributes.counter}`;//Question number
      const primaryText = new Alexa.RichTextContentHelper().withPrimaryText(getQuestionWithoutOrdinal(property, item)).getTextContent();//1 and 2 passed as attr, question text
      const backgroundImage = new Alexa.ImageHelper().addImageInstance(getBackgroundImage(attributes.quizItem.Abbreviation)).getImage();//bg img
      const itemList = [];
      getAndShuffleMultipleChoiceAnswers(attributes.selectedItemIndex, item, property).forEach((x, i) => {//building a list of shuffled answers for question with multiple choices
        itemList.push(
          {
            "token" : x,//x is value of item and i is index of that item
            "textContent" : new Alexa.PlainTextContentHelper().withPrimaryText(x).getTextContent(),
          }
        );
      });
      response.addRenderTemplateDirective({//for display
        type : 'ListTemplate1',
        token : 'Question',
        backButton : 'hidden',
        backgroundImage,
...
const DefinitionHandler = {
  canHandle(handlerInput) {
    console.log("Inside DefinitionHandler");//access your logs is to go to Cloudwatch directly.there is a navigation bar with a link to "Logs."
